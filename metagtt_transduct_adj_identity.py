        ...
        for it in range(0, args.ITER + 1):
            ...
            for step in range(args.syn_steps):
                ...
                if step % 500 == 0:
                    ...
                    '''
                    feat_cal = feat_out.detach()
                    adj_syn_cal = torch.sigmoid(torch.matmul(feat_cal, feat_cal.t()))
                    logging.info(adj_syn_cal)
                    adj_syn_cal_norm = utils.normalize_adj_tensor(adj_syn_cal, sparse=False)
                    adj_syn_norm_key['0'] = adj_syn_cal_norm
                    #print(adj_syn_norm_key['0'])
                    print('here!')
                    '''
                    '''
                    feat_knn = F.normalize(feat_out.detach(), p=1).to(self.device)
                    adj_syn_cal = torch.from_numpy(self.nearest_neighbors(feat_knn, args.k)).to(self.device)
                    adj_syn_mix = adj_syn_cal + self.adj_syn_init
                    adj_syn_mix[torch.where(adj_syn_mix.gt(1))] = 1
                    adj_syn_key['0'] = adj_syn_mix
                    adj_syn_cal_norm = utils.normalize_adj_tensor(adj_syn_mix, sparse=False)
                    adj_syn_norm_key['0'] = adj_syn_cal_norm
                    #logging.info(adj_syn_norm_key['0'])
                    '''
