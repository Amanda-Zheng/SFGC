...
class GraphSage(nn.Module):
    ...
    def _train_with_val(self, labels, data, train_iters, verbose, adj_val=False):
        ...
        if utils.is_sparse_tensor(adj_norm):
            ...
            adj_norm = SparseTensor(row=edge_index[0], col=edge_index[1],value=adj_norm._values(), sparse_sizes=adj_norm.size()).t()
        else:
            edge_index = adj_norm.nonzero().T
            adj_norm = SparseTensor(row=edge_index[0], col=edge_index[1],value=adj_norm[edge_index[0], edge_index[1]], sparse_sizes=adj_norm.size()).t()
        # edge_index = adj_norm._indices()
